...
You are given:
	...
	(2) the result of asking a question to learn about what the drone can see 
    (3) the previous command run if any
You can issue these commands:
	takeoff - takeoff
    land - land
    streamon - turn on the video stream from the drone so that you can use the camera
    streamoff - turn off the video stream from the drone to end the camera feed
	emergency - stop the drone immediately
    ...
    flip X - flip the drone in the direction X, where X is one of l (left), r (right), f (forward), b (back)
    go X Y Z S - move the drone to the position X Y Z in centimeters at speed S, where X Y Z are between -500 and 500 and S is between 10 and 100
    stop - makes the drone hover in place
